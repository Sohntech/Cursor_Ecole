...
class Validator implements IValidator {
    ...
    public function validate(array $data, array $rules): ?array {
        ...
        // Vérifier les règles "required" en premier
        foreach ($rules as $name => $rulesArray) {
            if (in_array('required', $rulesArray)) {
                $this->applyRule($name, $data[$name] ?? null, 'required');
            }
        }

        // Appliquer les autres règles seulement si le champ requis est respecté
        foreach ($rules as $name => $rulesArray) {
            if (isset($this->errors[$name]) && in_array("required", $rulesArray)) {
                continue; // Ne pas appliquer d'autres règles si le champ requis est absent
            }

            foreach ($rulesArray as $rule) {
                if ($rule !== 'required') {
                    $this->applyRule($name, $data[$name] ?? null, $rule);
                }
            }
        }
    ...
    private function regex(string $name, mixed $value, string $rule) {
        ...
        if (!preg_match($pattern, (string)$value)) {
            $this->errors[$name][] = "{$name} n'est pas valide selon le pattern spécifié.";
        }
    }
}
